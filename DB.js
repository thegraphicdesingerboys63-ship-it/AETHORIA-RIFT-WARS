...
async function runMigrations() {
  // Add new columns — silently ignore if they already exist
  const cols = [
    ...
    'ALTER TABLE users ADD COLUMN two_fa_code_expires INTEGER',
  ];
  let isNewColumn = false;
  for (const sql of cols) {
    try { await db.execute(sql); if (sql.includes('email_verified')) isNewColumn = true; } catch {}
  }
  // Only grandfather existing users the first time this column is added
  if (isNewColumn) {
    await db.execute({ sql: 'UPDATE users SET email_verified=1 WHERE email_verified=0', args: [] });
    console.log('[DB] Grandfathered existing users as email-verified');
  }
  ...
  if (existing.rows.length === 0) {
    ...
    await db.execute({
      sql: `INSERT INTO users (username, password_hash, email, email_verified, role, currency, developer_mode)
            VALUES (?, ?, ?, 1, ?, ?, ?)`,
      args: ['AMGProdZ', hash, 'aidenplayzmail@gmail.com', 'developer', 999999, 1],
    });
    ...
    await db.execute({
      sql: `UPDATE users SET email='aidenplayzmail@gmail.com', email_verified=1 WHERE username='AMGProdZ' AND (email IS NULL OR email='dev@aethoria.gg')`,
      args: [],
    });
  }
...
export async function createUser(username, passwordHash, email) {
  const r = await db.execute({
    sql: `INSERT INTO users (username, password_hash, email, email_verified) VALUES (?,?,?,0) RETURNING id`,
