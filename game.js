...
function drawHUD() {
  for (let i = 0; i < players.length; i++) {
    ...
    drawPixelBorder(bx, by, panelW, panelH, elColor);
    ...
    for (let s = 0; s < STOCK_COUNT; s++) {
      ...
      drawPixelStockIcon(sx, by + 35, 6, s < p.stocks, elColor);
    }
    ...
    for (let seg = 0; seg < segCount; seg++) {
      ...
      if (seg < filledSegs) {
        const segColor = ulFull ? (frame % 12 < 6 ? '#FFD700' : '#FFFFFF') : elColor;
...
function renderSpriteToCanvas(charData, destCanvas) {
  const SCALE = 3;
  const d = charData;
  const pal = d.palette || [];
...
function renderCharSelect(mode) {
  ...
  for (const char of CHARACTER_LIST) {
    ...
    card.style.setProperty('--el-color', elColor);

    const portraitDiv = document.createElement('div');
    portraitDiv.className = 'char-portrait';
    portraitDiv.style.background = elColor + '18';
    ...
    if (!locked) card.addEventListener('click', () => selectChar(char.id, mode));
    grid.appendChild(card);
  }
}

let p1Select = null;
function selectChar(charId, mode) {
  if (!p1Select) {
    p1Select = charId;
...
async function loadShop() {
  ...
  for (const c of cosmetics) {
    ...
    item.innerHTML = `
      <div class="shop-item-preview" style="--el:${elColor}">
        <canvas class="shop-item-canvas" width="80" height="80" data-char="${c.character_id}" data-type="${c.type}"></canvas>
  ...
  requestAnimationFrame(() => {
    document.querySelectorAll('.shop-item-canvas').forEach(cvs => {
      ...
      // BG
      c.fillStyle = elColor + '22';
      c.fillRect(0, 0, 80, 80);
      if (char) {
        renderSpriteToCanvas(char, cvs);
      } else {
        // ALL characters — draw element symbol
        drawCosmeticIcon(c, type, elColor, 80, 80);
      }
    });
  });
...
function drawCosmeticIcon(c, type, color, w, h) {
  ...
  const icons = { skin:'◈', trail:'✦', hiteffect:'⚡', aura:'◉', default:'◆' };
  c.fillText(icons[type] || icons.default, cx, cy);
  // Glow
  c.globalAlpha = 0.25;
  c.fillStyle = color;
  c.fillRect(0, 0, w, h);
...
export function init() {
  ...
  if (savedToken) {
    ...
    fetch('/api/me', { headers: { Authorization: `Bearer ${token}` } })
      .then(r => r.json())
      .then(data => {
        if (data.error) { localStorage.removeItem('rift_token'); token = null; return; }
  ...
  window.gameAPI = {
    login, register, playAsGuest, renderMenu, renderCharSelect, startGame, startStory, advanceStory,
