...
app.post('/api/register', async (req, res) => {
  ...
  try {
    ...
    await DB.setBackupCode(id, backupHash);
    res.json({ needs_backup_save: true, backup_code: backupCode });
  } catch (e) {
...
app.post('/api/login', async (req, res) => {
  ...
  try {
    ...
    // Admin/developer/moderator bypass all verification
    if (['admin', 'developer', 'moderator'].includes(user.role)) {
      await DB.updateLastLogin(user.id);
      const token = jwt.sign({ id: user.id, username: user.username, role: user.role }, JWT_SECRET, { expiresIn: '7d' });
      return res.json({ token, user: { id: user.id, username: user.username, role: user.role, currency: user.currency, custom_title: user.custom_title, developer_mode: user.developer_mode, two_fa_enabled: !!user.two_fa_enabled, email: user.email, email_verified: true } });
    }
    // Regular users — require admin approval or backup code
    if (onlineAdmins.size > 0) {
      const requestId = uuidv4();
      pendingLoginRequests.set(requestId, { userId: user.id, username: user.username, approved: null, created_at: Date.now() });
      for (const adminId of onlineAdmins) {
        io.to(`user:${adminId}`).emit('login_request', { requestId, username: user.username });
      }
      setTimeout(() => pendingLoginRequests.delete(requestId), 2 * 60 * 1000);
      return res.json({ requires_approval: true, request_id: requestId, username: user.username });
    }
    // No admin online — use backup code
    const partial = jwt.sign({ id: user.id, type: 'backup_pending' }, JWT_SECRET, { expiresIn: '10m' });
    return res.json({ requires_backup_code: true, partial_token: partial, username: user.username });
  } catch {
    res.status(500).json({ error: 'Server error' });
  }
});

// ─── LOGIN APPROVAL / BACKUP CODE ROUTES ─────────────────────────────────────
// Poll for approval status (regular users)
app.get('/api/login/check/:requestId', async (req, res) => {
  const entry = pendingLoginRequests.get(req.params.requestId);
  if (!entry) return res.json({ status: 'expired' });
  if (entry.approved === null) return res.json({ status: 'pending' });
  pendingLoginRequests.delete(req.params.requestId);
  if (!entry.approved) return res.json({ status: 'denied' });
  const user = await DB.getUserById(entry.userId);
  if (!user) return res.json({ status: 'expired' });
  await DB.updateLastLogin(user.id);
  const token = jwt.sign({ id: user.id, username: user.username, role: user.role }, JWT_SECRET, { expiresIn: '7d' });
  return res.json({ status: 'approved', token, user: { id: user.id, username: user.username, role: user.role, currency: user.currency, custom_title: user.custom_title, developer_mode: user.developer_mode, two_fa_enabled: !!user.two_fa_enabled, email: user.email, email_verified: true } });
});

// Admin approves or denies a pending login request
app.post('/api/login/respond', adminMiddleware, async (req, res) => {
  const { request_id, approved } = req.body;
  const entry = pendingLoginRequests.get(request_id);
  if (!entry) return res.status(404).json({ error: 'Request not found or expired' });
  entry.approved = !!approved;
  res.json({ success: true, username: entry.username });
});

// Verify backup code when admin is offline
app.post('/api/login/verify-backup', async (req, res) => {
  const { partial_token, backup_code } = req.body;
  if (!partial_token || !backup_code) return res.status(400).json({ error: 'Missing fields' });
  let payload;
  ...
  if (payload.type !== 'backup_pending') return res.status(401).json({ error: 'Invalid token' });
  const user = await DB.getUserById(payload.id);
  if (!user) return res.status(404).json({ error: 'User not found' });
  if (!user.backup_code) return res.status(403).json({ error: 'No backup code set. An admin must be online to log you in.' });
  const hash = crypto.createHash('sha256').update(backup_code.trim().toUpperCase()).digest('hex');
  if (hash !== user.backup_code) return res.status(401).json({ error: 'Invalid backup code' });
  await DB.updateLastLogin(user.id);
  const token = jwt.sign({ id: user.id, username: user.username, role: user.role }, JWT_SECRET, { expiresIn: '7d' });
  res.json({ token, user: { id: user.id, username: user.username, role: user.role, currency: user.currency, custom_title: user.custom_title, developer_mode: user.developer_mode, two_fa_enabled: !!user.two_fa_enabled, email: user.email, email_verified: true } });
});

// Regenerate backup code (must be logged in)
app.post('/api/account/regenerate-backup', authMiddleware, async (req, res) => {
  const newCode = generateBackupCode();
  const hash = crypto.createHash('sha256').update(newCode).digest('hex');
  await DB.setBackupCode(req.user.id, hash);
  res.json({ backup_code: newCode });
});
...
const adminCmds = {
  ...
  // ─── 100 FUN DEV COMMANDS ─────────────────────────────────────────────────

  // ECONOMY & CURRENCY
  set_everyone_currency: async (admin, { amount }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    await DB.db.execute({ sql: 'UPDATE users SET currency=?', args: [amount] });
    io.emit('broadcast', { message: `💰 Everyone's Rift Shards set to ${amount} by ${admin.username}.` });
    return { success: true };
  },
  bankrupt_everyone: async (admin) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    await DB.db.execute({ sql: 'UPDATE users SET currency=0', args: [] });
    io.emit('broadcast', { message: `💸 ECONOMIC COLLAPSE — everyone is broke. Thanks, ${admin.username}.` });
    return { success: true };
  },
  make_billionaires: async (admin) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    await DB.db.execute({ sql: 'UPDATE users SET currency=1000000', args: [] });
    io.emit('broadcast', { message: `🤑 EVERYONE IS A BILLIONAIRE — ${admin.username} has ruined the economy.` });
    return { success: true };
  },
  tax_everyone: async (admin, { percent }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    const pct = Math.max(1, Math.min(99, parseInt(percent) || 10));
    await DB.db.execute({ sql: `UPDATE users SET currency=MAX(0, CAST(currency*(1-${pct/100}) AS INTEGER))`, args: [] });
    io.emit('broadcast', { message: `🧾 ${admin.username} has imposed a ${pct}% tax. Pay up.` });
    return { success: true, taxRate: pct };
  },
  double_everyone_currency: async (admin) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    await DB.db.execute({ sql: 'UPDATE users SET currency=MIN(currency*2, 9999999)', args: [] });
    io.emit('broadcast', { message: `✨ DOUBLE MONEY EVENT — ${admin.username} doubled everyone's Rift Shards!` });
    return { success: true };
  },
  halve_everyone_currency: async (admin) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    await DB.db.execute({ sql: 'UPDATE users SET currency=MAX(0,currency/2)', args: [] });
    io.emit('broadcast', { message: `📉 ${admin.username} halved everyone's wallet. Rough day.` });
    return { success: true };
  },
  steal_from_rich: async (admin, { amount }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    const amt = parseInt(amount) || 100;
    const rich = await DB.db.execute({ sql: `SELECT id FROM users WHERE currency>${amt*10} ORDER BY currency DESC LIMIT 10`, args: [] });
    const poor = await DB.db.execute({ sql: `SELECT id FROM users WHERE currency<${amt} ORDER BY currency ASC LIMIT 10`, args: [] });
    for (const u of rich.rows) await DB.updateCurrency(u.id, -amt);
    for (const u of poor.rows) await DB.updateCurrency(u.id, amt);
    io.emit('broadcast', { message: `🏹 Robin Hood Mode — ${admin.username} redistributed wealth!` });
    return { success: true, taken: rich.rows.length, given: poor.rows.length };
  },
  jackpot: async (admin) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    const users = await DB.getAllUsers(9999, 0);
    ...
    if (!winner) return { error: 'No users' };
    await DB.updateCurrency(winner.id, 50000);
    io.emit('broadcast', { message: `🎰 JACKPOT! ${winner.username} won 50,000 Rift Shards from the dev lottery!` });
    return { success: true, winner: winner.username };
  },
  currency_heist: async (admin) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    const users = await DB.getAllUsers(9999, 0);
    const list = (users.users || users).filter(u => u.id !== admin.id);
    let stolen = 0;
    for (const u of list) {
      const take = Math.floor((u.currency || 0) * 0.1);
      if (take > 0) { await DB.updateCurrency(u.id, -take); await DB.updateCurrency(admin.id, take); stolen += take; }
    }
    io.emit('broadcast', { message: `🦹 HEIST! ${admin.username} stole 10% from everyone's wallet.` });
    return { success: true, totalStolen: stolen };
  },
  mystery_box: async (admin, { userId }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    const outcomes = [
      { msg: '🎁 You got 5000 Rift Shards!', amount: 5000 },
      { msg: '💎 JACKPOT! 25000 Rift Shards!', amount: 25000 },
      { msg: '😢 Nothing. The box was empty.', amount: 0 },
      { msg: '🪲 A bug crawled out. -100 Shards.', amount: -100 },
      { msg: '🔮 Cosmic reward: 10000 Rift Shards!', amount: 10000 },
    ];
    const pick = outcomes[Math.floor(Math.random() * outcomes.length)];
    if (pick.amount !== 0) await DB.updateCurrency(userId, pick.amount);
    io.to(`user:${userId}`).emit('system_message', { message: `Mystery Box: ${pick.msg}` });
    return { success: true, outcome: pick };
  },

  // PLAYER EFFECTS
  freeze_player: async (admin, { userId, duration }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    io.to(`user:${userId}`).emit('player_effect', { type: 'freeze', duration: duration || 5000 });
    io.emit('broadcast', { message: `🧊 ${admin.username} froze a player solid!` });
    return { success: true };
  },
  shrink_player: async (admin, { userId }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    io.to(`user:${userId}`).emit('player_effect', { type: 'shrink' });
    io.emit('broadcast', { message: `🔬 Someone just got TINY!` });
    return { success: true };
  },
  slow_player: async (admin, { userId, duration }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    io.to(`user:${userId}`).emit('player_effect', { type: 'slow', duration: duration || 8000 });
    return { success: true };
  },
  speed_player: async (admin, { userId }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    io.to(`user:${userId}`).emit('player_effect', { type: 'speed_boost', duration: 10000 });
    return { success: true };
  },
  teleport_player: async (admin, { userId, x, y }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    io.to(`user:${userId}`).emit('player_effect', { type: 'teleport', x: x || 400, y: y || 200 });
    return { success: true };
  },
  invisible_player: async (admin, { userId, duration }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    io.to(`user:${userId}`).emit('player_effect', { type: 'invisible', duration: duration || 5000 });
    return { success: true };
  },
  stun_player: async (admin, { userId }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    io.to(`user:${userId}`).emit('player_effect', { type: 'stun', duration: 3000 });
    return { success: true };
  },
  rainbow_player: async (admin, { userId }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    io.to(`user:${userId}`).emit('player_effect', { type: 'rainbow', duration: 15000 });
    io.emit('broadcast', { message: `🌈 Someone's glowing with rainbow energy!` });
    return { success: true };
  },
  give_player_wings: async (admin, { userId }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    io.to(`user:${userId}`).emit('player_effect', { type: 'infinite_jumps', duration: 30000 });
    return { success: true, message: 'Player has infinite jumps for 30 seconds.' };
  },
  give_player_godmode: async (admin, { userId }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    io.to(`user:${userId}`).emit('godmode', { enabled: true });
    return { success: true };
  },
  remove_player_godmode: async (admin, { userId }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    io.to(`user:${userId}`).emit('godmode', { enabled: false });
    return { success: true };
  },
  knockback_everyone: async (admin) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    io.emit('world_event', { eventType: 'shockwave', duration: 1000, data: {} });
    io.emit('broadcast', { message: `💥 ${admin.username} sent out a shockwave!` });
    return { success: true };
  },

  // WORLD EVENTS
  earthquake: async (admin) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    io.emit('world_event', { eventType: 'earthquake', duration: 5000, data: {} });
    io.emit('broadcast', { message: `🌍 EARTHQUAKE — the stages are shaking!` });
    await DB.logAdminAction(admin.id, 'earthquake', null, {});
    return { success: true };
  },
  lightning_storm: async (admin) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    io.emit('world_event', { eventType: 'lightning_storm', duration: 20000, data: {} });
    io.emit('broadcast', { message: `⚡ LIGHTNING STORM — Neon Ryu approves!` });
    return { success: true };
  },
  blizzard: async (admin) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    io.emit('world_event', { eventType: 'blizzard', duration: 30000, data: {} });
    io.emit('broadcast', { message: `❄️ BLIZZARD — everything is slowed!` });
    return { success: true };
  },
  sandstorm: async (admin) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    io.emit('world_event', { eventType: 'sandstorm', duration: 20000, data: {} });
    io.emit('broadcast', { message: `🌪️ SANDSTORM — visibility reduced!` });
    return { success: true };
  },
  acid_rain: async (admin) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    io.emit('world_event', { eventType: 'acid_rain', duration: 15000, data: {} });
    io.emit('broadcast', { message: `☠️ ACID RAIN — the stage is melting!` });
    return { success: true };
  },
  eclipse: async (admin) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    io.emit('world_event', { eventType: 'eclipse', duration: 60000, data: {} });
    io.emit('broadcast', { message: `🌑 RIFT ECLIPSE — the sky goes dark for 60 seconds.` });
    return { success: true };
  },
  void_rift_tear: async (admin) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    io.emit('world_event', { eventType: 'void_rift', duration: 30000, data: {} });
    io.emit('broadcast', { message: `🌀 A VOID RIFT tears through the arena — anything could come out.` });
    return { success: true };
  },
  confetti_storm: async (admin) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    io.emit('world_event', { eventType: 'confetti', duration: 10000, data: {} });
    io.emit('broadcast', { message: `🎊 IT'S A PARTY — ${admin.username} popped the confetti cannon!` });
    return { success: true };
  },
  fire_rain: async (admin) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    io.emit('world_event', { eventType: 'fire_rain', duration: 20000, data: {} });
    io.emit('broadcast', { message: `🔥 FIRE RAIN — Turbo Kid's nightmare just became everyone's problem.` });
    return { success: true };
  },
  time_stop: async (admin, { duration }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    const ms = Math.min(parseInt(duration) || 5000, 30000);
    io.emit('world_event', { eventType: 'time_stop', duration: ms, data: {} });
    io.emit('broadcast', { message: `⏸️ TIME STOP — ${admin.username} froze the clock for ${ms/1000}s!` });
    return { success: true };
  },
  stage_flip: async (admin) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    globalFlags.stageFlipped = !globalFlags.stageFlipped;
    io.emit('flag_update', globalFlags);
    io.emit('broadcast', { message: `🔄 Stage flipped ${globalFlags.stageFlipped ? 'horizontally' : 'back'} by ${admin.username}!` });
    return { success: true };
  },
  low_gravity: async (admin, { enabled }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    globalFlags.lowGravity = !!enabled;
    io.emit('flag_update', globalFlags);
    io.emit('broadcast', { message: enabled ? '🌙 LOW GRAVITY — everyone floats now!' : '🌍 Normal gravity restored.' });
    return { success: true };
  },
  turbo_gravity: async (admin, { enabled }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    globalFlags.turboGravity = !!enabled;
    io.emit('flag_update', globalFlags);
    io.emit('broadcast', { message: enabled ? '⬇️ TURBO GRAVITY — good luck jumping.' : 'Normal gravity restored.' });
    return { success: true };
  },
  no_floor: async (admin, { enabled }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    globalFlags.noFloor = !!enabled;
    io.emit('flag_update', globalFlags);
    io.emit('broadcast', { message: enabled ? `🕳️ THE FLOOR IS GONE — ${admin.username} removed the stage!` : '✅ Stage floor restored.' });
    return { success: true };
  },

  // SCORES & RANKS
  reset_my_stats: async (admin) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    await DB.db.execute({ sql: 'UPDATE users SET total_wins=0,total_losses=0,total_matches=0 WHERE id=?', args: [admin.id] });
    await DB.db.execute({ sql: 'UPDATE rankings SET elo=1000,wins=0,losses=0,draws=0,season_elo=1000 WHERE user_id=?', args: [admin.id] });
    return { success: true, message: 'Your stats wiped clean.' };
  },
  crown_yourself: async (admin) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    await DB.db.execute({ sql: 'UPDATE rankings SET elo=9999 WHERE user_id=?', args: [admin.id] });
    await DB.setCustomTitle(admin.id, '👑 THE DEVELOPER');
    io.emit('broadcast', { message: `👑 ${admin.username} has crowned themselves #1. Power move.` });
    return { success: true };
  },
  set_player_elo: async (admin, { userId, elo }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    await DB.db.execute({ sql: 'UPDATE rankings SET elo=?,peak_elo=MAX(peak_elo,?) WHERE user_id=?', args: [elo, elo, userId] });
    return { success: true };
  },
  promote_to_legend: async (admin, { userId }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    await DB.db.execute({ sql: 'UPDATE rankings SET elo=3000 WHERE user_id=?', args: [userId] });
    await DB.setCustomTitle(userId, '⚡ LEGEND');
    const user = await DB.getUserById(userId);
    io.emit('broadcast', { message: `⚡ ${user?.username || 'A player'} has been promoted to LEGEND rank by ${admin.username}!` });
    return { success: true };
  },
  demote_to_bronze: async (admin, { userId }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    await DB.db.execute({ sql: 'UPDATE rankings SET elo=100,wins=0,losses=1 WHERE user_id=?', args: [userId] });
    return { success: true };
  },
  inflate_everyone_elo: async (admin, { amount }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    const amt = parseInt(amount) || 100;
    await DB.db.execute({ sql: 'UPDATE rankings SET elo=elo+?', args: [amt] });
    io.emit('broadcast', { message: `📈 ELO INFLATION — everyone gained ${amt} ELO. Season integrity: questionable.` });
    return { success: true };
  },
  deflate_everyone_elo: async (admin, { amount }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    const amt = parseInt(amount) || 100;
    await DB.db.execute({ sql: 'UPDATE rankings SET elo=MAX(0,elo-?)', args: [amt] });
    io.emit('broadcast', { message: `📉 ELO DEFLATION — everyone lost ${amt} ELO. Season integrity: still questionable.` });
    return { success: true };
  },
  give_everyone_wins: async (admin, { count }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    const n = parseInt(count) || 10;
    await DB.db.execute({ sql: 'UPDATE rankings SET wins=wins+?,elo=elo+?', args: [n, n*10] });
    io.emit('broadcast', { message: `🏆 ${admin.username} gifted everyone ${n} free wins!` });
    return { success: true };
  },

  // COSMETICS & TITLES
  strip_all_cosmetics: async (admin) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    await DB.db.execute({ sql: 'DELETE FROM user_cosmetics', args: [] });
    io.emit('broadcast', { message: `👕 ${admin.username} stripped everyone's cosmetics. Default skins only.` });
    return { success: true };
  },
  give_everyone_rare_cosmetics: async (admin) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    const rares = await DB.db.execute({ sql: "SELECT id FROM cosmetics WHERE rarity='rare'", args: [] });
    const users = await DB.getAllUsers(9999, 0);
    const list = users.users || users;
    let count = 0;
    for (const u of list) {
      for (const c of rares.rows) { await DB.grantCosmetic(u.id, c.id).catch(() => {}); count++; }
    }
    io.emit('broadcast', { message: `💜 ALL RARE COSMETICS unlocked for everyone by ${admin.username}!` });
    return { success: true, granted: count };
  },
  give_everyone_special_cosmetics: async (admin) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    const specials = await DB.db.execute({ sql: "SELECT id FROM cosmetics WHERE is_special=1", args: [] });
    const users = await DB.getAllUsers(9999, 0);
    const list = users.users || users;
    for (const u of list) for (const c of specials.rows) await DB.grantCosmetic(u.id, c.id).catch(() => {});
    io.emit('broadcast', { message: `🌟 SPECIAL COSMETICS granted to all players by ${admin.username}!` });
    return { success: true };
  },
  wipe_all_titles: async (admin) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    await DB.db.execute({ sql: 'UPDATE users SET custom_title=NULL', args: [] });
    io.emit('broadcast', { message: `🗑️ All custom titles wiped by ${admin.username}.` });
    return { success: true };
  },
  give_everyone_title: async (admin, { title }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    await DB.db.execute({ sql: 'UPDATE users SET custom_title=?', args: [title] });
    io.emit('broadcast', { message: `🏅 Everyone is now titled: [${title}]` });
    return { success: true };
  },
  random_title_everyone: async (admin) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    const titles = ['Rift Drifter','Void Walker','Neon Ghost','Rift Breaker','Pixel Phantom','Circuit Sage','Storm Rider','Null Hunter','Rift Sworn','Arcane Exile','Static Rebel','Echo Fighter','Void Born','Chaos Agent','Rift Herald'];
    const users = await DB.getAllUsers(9999, 0);
    for (const u of (users.users || users)) {
      await DB.setCustomTitle(u.id, titles[Math.floor(Math.random() * titles.length)]);
    }
    io.emit('broadcast', { message: `🎲 ${admin.username} randomized everyone's titles!` });
    return { success: true };
  },

  // SERVER & SYSTEM
  lock_registrations: async (admin, { enabled }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    globalFlags.registrationLocked = !!enabled;
    io.emit('flag_update', globalFlags);
    return { success: true, locked: globalFlags.registrationLocked };
  },
  set_max_players: async (admin, { max }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    globalFlags.maxPlayers = parseInt(max) || 100;
    io.emit('flag_update', globalFlags);
    return { success: true, maxPlayers: globalFlags.maxPlayers };
  },
  toggle_ranked: async (admin) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    globalFlags.ranked = !globalFlags.ranked;
    io.emit('flag_update', globalFlags);
    io.emit('broadcast', { message: `${globalFlags.ranked ? '🏆 Ranked mode RE-ENABLED.' : '🚫 Ranked mode DISABLED.'}` });
    return { success: true, ranked: globalFlags.ranked };
  },
  toggle_combos: async (admin) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    globalFlags.comboSystem = !globalFlags.comboSystem;
    io.emit('flag_update', globalFlags);
    io.emit('broadcast', { message: `🎮 Combo system ${globalFlags.comboSystem ? 'enabled' : 'disabled'} by ${admin.username}.` });
    return { success: true };
  },
  toggle_elemental_damage: async (admin) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    globalFlags.elementalDamage = !globalFlags.elementalDamage;
    io.emit('flag_update', globalFlags);
    io.emit('broadcast', { message: `🔥 Elemental damage ${globalFlags.elementalDamage ? 'enabled' : 'disabled'}.` });
    return { success: true };
  },
  toggle_friendly_fire: async (admin) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    globalFlags.friendlyFire = !globalFlags.friendlyFire;
    io.emit('flag_update', globalFlags);
    io.emit('broadcast', { message: `⚔️ Friendly fire is now ${globalFlags.friendlyFire ? 'ON' : 'OFF'}.` });
    return { success: true };
  },
  toggle_infinite_stocks: async (admin) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    globalFlags.infiniteStocks = !globalFlags.infiniteStocks;
    io.emit('flag_update', globalFlags);
    io.emit('broadcast', { message: `♾️ Infinite stocks ${globalFlags.infiniteStocks ? 'ON' : 'OFF'} — ${admin.username} changed the rules.` });
    return { success: true };
  },
  toggle_one_hit_ko: async (admin) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    globalFlags.oneHitKO = !globalFlags.oneHitKO;
    io.emit('flag_update', globalFlags);
    io.emit('broadcast', { message: `💀 ONE HIT KO mode is ${globalFlags.oneHitKO ? 'ON — pray.' : 'OFF.'}` });
    return { success: true };
  },
  toggle_no_damage: async (admin) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    globalFlags.noDamage = !globalFlags.noDamage;
    io.emit('flag_update', globalFlags);
    io.emit('broadcast', { message: `🛡️ No damage mode ${globalFlags.noDamage ? 'ON' : 'OFF'}.` });
    return { success: true };
  },
  toggle_mirror_mode: async (admin) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    globalFlags.mirrorMode = !globalFlags.mirrorMode;
    io.emit('flag_update', globalFlags);
    io.emit('broadcast', { message: `🪞 Mirror mode ${globalFlags.mirrorMode ? 'ON — your character copies every move.' : 'OFF.'}` });
    return { success: true };
  },
  warp_speed: async (admin, { multiplier }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    const mult = Math.max(0.1, Math.min(10, parseFloat(multiplier) || 2));
    globalFlags.speedMultiplier = mult;
    io.emit('flag_update', globalFlags);
    io.emit('broadcast', { message: `💨 Game speed set to ${mult}x by ${admin.username}.` });
    return { success: true, speed: mult };
  },
  reset_speed: async (admin) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    globalFlags.speedMultiplier = 1;
    io.emit('flag_update', globalFlags);
    io.emit('broadcast', { message: `🕐 Speed reset to 1x.` });
    return { success: true };
  },

  // FUN MESSAGES & TAUNTS
  taunt_player: async (admin, { userId, taunt }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    const taunts = [
      'L + ratio + no recovery',
      'skill issue detected',
      'the dev is watching you',
      'your neutral game is suffering',
      'lag? sure. definitely lag.',
      'you\'ve been formally challenged by the developer',
      'ratio',
    ];
    const msg = taunt || taunts[Math.floor(Math.random() * taunts.length)];
    io.to(`user:${userId}`).emit('system_message', { message: `[DEV] ${msg}` });
    return { success: true, message: msg };
  },
  broadcast_lore: async (admin, { character }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    const lore = {
      NEON_RYU: 'The night Voltcity went dark, Ryu stopped fearing the lightning. He became it.',
      VHS_VIPER: 'She exists between frames. The tape always rewinds to the moment before she strikes.',
      ARCADE_TITAN: 'Cabinet Unit 3-UP has 40 years of match data. It has never lost the same way twice.',
      TURBO_KID: 'He was 12 when the Rift Colosseum opened its gates. He was 13 when he won it.',
      PIXEL_PIRATE: 'Captain Hex once stole a rift shard mid-combat and used it as a cannonball.',
      FROST_VALKYRIE: 'She sealed her first rift with a spear and sheer contempt for the void.',
      SHADOW_BEAST: 'The Null does not destroy. It erases. There is a difference.',
    };
    const char = character?.toUpperCase();
    const text = lore[char] || `The rift has many stories. Not all of them survive the telling.`;
    io.emit('broadcast', { message: `📜 [LORE] ${text}` });
    await DB.logAdminAction(admin.id, 'broadcast_lore', null, { character });
    return { success: true, lore: text };
  },
  dev_riddle: async (admin) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    const riddles = [
      'I have no body but I end every match. What am I? (The blast zone)',
      'I fill as you fight and empty in an instant. What am I? (The Rift Meter)',
      'I bypass shields and end friendships. What am I? (The Grab)',
      'I am faster than a jab but weaker than a smash. What am I? (A tilt)',
      'I deal damage and pull the opponent in when Lightning meets Earth. What am I? (A Magnetize reaction)',
    ];
    const pick = riddles[Math.floor(Math.random() * riddles.length)];
    io.emit('broadcast', { message: `🧩 [DEV RIDDLE] ${pick}` });
    return { success: true, riddle: pick };
  },
  countdown: async (admin, { seconds, message }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    const secs = Math.min(parseInt(seconds) || 10, 60);
    const msg = message || 'Something is coming.';
    for (let i = secs; i >= 1; i--) {
      setTimeout(() => io.emit('broadcast', { message: `⏱️ [${msg}] — ${i}` }), (secs - i) * 1000);
    }
    setTimeout(() => io.emit('broadcast', { message: `💥 [${msg}] — NOW!` }), secs * 1000);
    return { success: true, countdown: secs };
  },
  fake_maintenance: async (admin, { duration }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    const ms = Math.min(parseInt(duration) || 10000, 60000);
    io.emit('broadcast', { message: `⚠️ Server entering maintenance in 10 seconds...` });
    setTimeout(() => io.emit('broadcast', { message: `✅ Maintenance complete. Server is fine. It was a drill.` }), ms);
    return { success: true };
  },
  announce_fake_update: async (admin, { version }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    const ver = version || `v${Math.floor(Math.random()*3)+1}.${Math.floor(Math.random()*9)}.${Math.floor(Math.random()*9)}`;
    io.emit('broadcast', { message: `🆕 AETHORIA: RIFT WARS ${ver} is now live! Patch notes: bug fixes, balance changes, and one thing we're not telling you about.` });
    return { success: true, version: ver };
  },

  // MATCH MANIPULATION
  end_all_matches: async (admin) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    for (const [roomId] of gameRooms) {
      io.to(`room:${roomId}`).emit('force_disconnect', { reason: 'Match ended by admin' });
      gameRooms.delete(roomId);
    }
    io.emit('broadcast', { message: `🛑 ${admin.username} ended all active matches.` });
    return { success: true, ended: gameRooms.size };
  },
  set_match_time_limit: async (admin, { minutes }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    globalFlags.matchTimeLimitMinutes = Math.max(1, Math.min(60, parseInt(minutes) || 8));
    io.emit('flag_update', globalFlags);
    return { success: true, minutes: globalFlags.matchTimeLimitMinutes };
  },
  pause_all_matchmaking: async (admin) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    globalFlags.matchmakingPaused = true;
    io.emit('flag_update', globalFlags);
    io.emit('broadcast', { message: `⏸️ Matchmaking paused by ${admin.username}.` });
    return { success: true };
  },
  resume_matchmaking: async (admin) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    globalFlags.matchmakingPaused = false;
    io.emit('flag_update', globalFlags);
    io.emit('broadcast', { message: `▶️ Matchmaking resumed.` });
    return { success: true };
  },
  force_rematch: async (admin, { userId1, userId2 }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    io.to(`user:${userId1}`).emit('system_message', { message: `[DEV] Rematch initiated!` });
    io.to(`user:${userId2}`).emit('system_message', { message: `[DEV] Rematch initiated!` });
    return { success: true };
  },

  // DATABASE TOOLS
  count_users: async (admin) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    const r = await DB.db.execute({ sql: 'SELECT COUNT(*) as total FROM users', args: [] });
    return { total: r.rows[0].total };
  },
  wipe_match_history: async (admin) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    await DB.db.execute({ sql: 'DELETE FROM matches', args: [] });
    return { success: true, message: 'All match history wiped.' };
  },
  backup_user_data: async (admin, { userId }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    const user = await DB.getUserById(userId);
    const ranking = await DB.getRanking(userId);
    const cosmetics = await DB.getUserCosmetics(userId);
    const history = await DB.getMatchHistory(userId, 50);
    return { user: { ...user, password_hash: '[HIDDEN]' }, ranking, cosmetics, historyCount: history.length };
  },
  audit_economy: async (admin) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    const r = await DB.db.execute({
      sql: `SELECT SUM(currency) as total_circulation, AVG(currency) as avg, MAX(currency) as richest, MIN(currency) as poorest, COUNT(*) as players FROM users`,
      args: [],
    });
    return r.rows[0];
  },
  get_top_earners: async (admin, { limit }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    const r = await DB.db.execute({ sql: 'SELECT username,currency FROM users ORDER BY currency DESC LIMIT ?', args: [parseInt(limit) || 10] });
    return r.rows;
  },
  get_newest_players: async (admin, { limit }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    const r = await DB.db.execute({ sql: 'SELECT username,created_at,role FROM users ORDER BY created_at DESC LIMIT ?', args: [parseInt(limit) || 10] });
    return r.rows;
  },
  find_inactive_players: async (admin, { days }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    const d = parseInt(days) || 30;
    const r = await DB.db.execute({ sql: `SELECT username,last_login FROM users WHERE last_login < datetime('now','-${d} days') OR last_login IS NULL ORDER BY last_login ASC LIMIT 50`, args: [] });
    return { inactive: r.rows, days: d };
  },
  purge_inactive: async (admin, { days }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    const d = parseInt(days) || 90;
    const r = await DB.db.execute({ sql: `DELETE FROM users WHERE role='player' AND (last_login < datetime('now','-${d} days') OR last_login IS NULL) AND id != ?`, args: [admin.id] });
    return { success: true, purged: r.rowsAffected, days: d };
  },

  // CREATIVE / LORE
  add_lore_entry: async (admin, { title, content }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    await DB.createAnnouncement(`[LORE] ${title}`, content, admin.id, false);
    io.emit('announcement', { title: `[LORE] ${title}`, content });
    return { success: true };
  },
  pin_announcement: async (admin, { id }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    await DB.db.execute({ sql: 'UPDATE announcements SET is_pinned=1 WHERE id=?', args: [id] });
    return { success: true };
  },
  unpin_announcement: async (admin, { id }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    await DB.db.execute({ sql: 'UPDATE announcements SET is_pinned=0 WHERE id=?', args: [id] });
    return { success: true };
  },
  clear_all_announcements: async (admin) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    await DB.db.execute({ sql: 'DELETE FROM announcements', args: [] });
    return { success: true };
  },
  dev_lore_drop: async (admin) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    const drops = [
      'The first rift appeared not in the sky — but in a loading screen that never ended.',
      'Aethoria was built on a hard drive that no one remembers formatting.',
      'The Null is not empty. It is full of every deleted save file.',
      'Shadow Beast was designed last. The dev was tired. It shows. Intentionally.',
      'The Verdant Throne was the first stage. The dev couldn\'t get the parallax right for three days.',
      'Glitch Witch\'s time loop special was accidentally added when the undo button broke.',
      'Pixel Pirate\'s treasure maps are real. No one has found them yet.',
    ];
    const pick = drops[Math.floor(Math.random() * drops.length)];
    io.emit('broadcast', { message: `📜 [DEV LORE DROP] ${pick}` });
    await DB.createAnnouncement('[Dev Lore Drop]', pick, admin.id, false);
    return { success: true, lore: pick };
  },
  secret_message: async (admin, { userId, message }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    io.to(`user:${userId}`).emit('system_message', { message: `🔐 [SECRET FROM DEV] ${message}` });
    return { success: true };
  },
  broadcast_riddle_answer: async (admin, { answer }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    io.emit('broadcast', { message: `✅ [RIDDLE ANSWER] ${answer}` });
    return { success: true };
  },
  dev_shoutout: async (admin, { userId }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    const user = await DB.getUserById(userId);
    if (!user) return { error: 'User not found' };
    io.emit('broadcast', { message: `🎤 [DEV SHOUTOUT] Big ups to ${user.username} — the developer sees you. Keep going.` });
    await DB.updateCurrency(userId, 500);
    return { success: true, shoutedOut: user.username };
  },
  send_riddle_event: async (admin, { riddle, answer, prize }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    const prizeAmt = parseInt(prize) || 1000;
    io.emit('broadcast', { message: `🧩 [RIDDLE EVENT] ${riddle} — First to answer in chat wins ${prizeAmt} Rift Shards!` });
    globalFlags.activeRiddle = { answer: answer?.toLowerCase(), prize: prizeAmt };
    io.emit('flag_update', globalFlags);
    return { success: true, riddle, prize: prizeAmt };
  },

  // MISC FUN
  play_sound_everyone: async (admin, { sound }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    io.emit('dev_sound', { sound: sound || 'rift_surge' });
    return { success: true };
  },
  flash_screen_everyone: async (admin, { color }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    io.emit('dev_flash', { color: color || '#ffffff', duration: 500 });
    return { success: true };
  },
  force_character: async (admin, { userId, characterId }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    io.to(`user:${userId}`).emit('player_effect', { type: 'force_character', characterId });
    return { success: true };
  },
  spawn_item: async (admin, { type, x, y }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    io.emit('world_event', { eventType: 'item_spawn', duration: 0, data: { type: type || 'rift_shard', x: x || 400, y: y || 300 } });
    return { success: true };
  },
  set_round_count: async (admin, { rounds }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    globalFlags.roundCount = Math.max(1, Math.min(9, parseInt(rounds) || 3));
    io.emit('flag_update', globalFlags);
    return { success: true, rounds: globalFlags.roundCount };
  },
  invert_controls_everyone: async (admin, { enabled }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    globalFlags.invertControls = !!enabled;
    io.emit('flag_update', globalFlags);
    io.emit('broadcast', { message: enabled ? `🔃 INVERTED CONTROLS — left is right and right is left, courtesy of ${admin.username}.` : '↩️ Controls restored.' });
    return { success: true };
  },
  random_stage_rotation: async (admin, { enabled }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    globalFlags.randomStages = !!enabled;
    io.emit('flag_update', globalFlags);
    io.emit('broadcast', { message: enabled ? `🗺️ RANDOM STAGE ROTATION enabled — every match gets a different stage!` : 'Stage selection back to normal.' });
    return { success: true };
  },
  enable_story_skip: async (admin, { enabled }) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    globalFlags.storySkip = !!enabled;
    io.emit('flag_update', globalFlags);
    return { success: true };
  },
  dev_stress_test: async (admin) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    const start = Date.now();
    const results = await Promise.all([
      DB.getServerStats(),
      DB.getLeaderboard(50),
      DB.getAnnouncements(),
      DB.db.execute({ sql: 'SELECT COUNT(*) as c FROM matches', args: [] }),
    ]);
    return { elapsed_ms: Date.now() - start, all_passed: results.every(r => !!r) };
  },

  nuke_and_rebuild: async (admin) => {
    if (admin.role !== 'developer') throw new Error('Developer only');
    // Wipe all non-essential data and broadcast a dramatic message
    await DB.db.execute({ sql: 'DELETE FROM matches', args: [] });
    await DB.db.execute({ sql: 'UPDATE rankings SET elo=1000,wins=0,losses=0,draws=0,season_elo=1000', args: [] });
    await DB.db.execute({ sql: 'DELETE FROM chat_logs', args: [] });
    await DB.db.execute({ sql: 'DELETE FROM user_cosmetics WHERE cosmetic_id NOT IN (SELECT id FROM cosmetics WHERE is_special=1)', args: [] });
    io.emit('broadcast', { message: `☢️ [${admin.username}] THE RIFT HAS BEEN RESET. All matches, chat, and ELO wiped. Season starts NOW.` });
    await DB.logAdminAction(admin.id, 'nuke_and_rebuild', null, {});
    return { success: true, message: 'Server reset complete. May the new season begin.' };
  },
};
...
app.get('/api/admin/commands', adminMiddleware, (req, res) => {
  ...
  const devOnlyCmds = ['delete_player_account','set_player_role','execute_sql','bulk_ban','bulk_grant_currency',
    ...
    // fun commands (all dev-only)
    'set_everyone_currency','bankrupt_everyone','make_billionaires','tax_everyone','double_everyone_currency',
    'halve_everyone_currency','steal_from_rich','jackpot','currency_heist','mystery_box',
    'freeze_player','shrink_player','slow_player','speed_player','teleport_player','invisible_player',
    'stun_player','rainbow_player','give_player_wings','give_player_godmode','remove_player_godmode','knockback_everyone',
    'earthquake','lightning_storm','blizzard','sandstorm','acid_rain','eclipse','void_rift_tear',
    'confetti_storm','fire_rain','time_stop','stage_flip','low_gravity','turbo_gravity','no_floor',
    'reset_my_stats','crown_yourself','set_player_elo','promote_to_legend','demote_to_bronze',
    'inflate_everyone_elo','deflate_everyone_elo','give_everyone_wins',
    'strip_all_cosmetics','give_everyone_rare_cosmetics','give_everyone_special_cosmetics',
    'wipe_all_titles','give_everyone_title','random_title_everyone',
    'lock_registrations','set_max_players','toggle_ranked','toggle_combos','toggle_elemental_damage',
    'toggle_friendly_fire','toggle_infinite_stocks','toggle_one_hit_ko','toggle_no_damage',
    'toggle_mirror_mode','warp_speed','reset_speed',
    'taunt_player','broadcast_lore','dev_riddle','countdown','fake_maintenance','announce_fake_update',
    'end_all_matches','set_match_time_limit','pause_all_matchmaking','resume_matchmaking','force_rematch',
    'count_users','wipe_match_history','backup_user_data','audit_economy','get_top_earners',
    'get_newest_players','find_inactive_players','purge_inactive',
    'add_lore_entry','pin_announcement','unpin_announcement','clear_all_announcements',
    'dev_lore_drop','secret_message','broadcast_riddle_answer','dev_shoutout','send_riddle_event',
    'play_sound_everyone','flash_screen_everyone','force_character','spawn_item','set_round_count',
    'invert_controls_everyone','random_stage_rotation','enable_story_skip','dev_stress_test','nuke_and_rebuild',
  ];
...
const onlineAdmins = new Set(); // user IDs of connected admin/dev/mod accounts
const pendingLoginRequests = new Map(); // requestId → { userId, username, approved, created_at }
