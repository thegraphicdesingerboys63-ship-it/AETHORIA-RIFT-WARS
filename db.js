...
async function seedData() {
  ...
  if (existing.rows.length === 0) {
    ...
    await db.execute({
      sql: `INSERT INTO users (username, password_hash, email, role, currency, developer_mode)
            VALUES (?, ?, ?, ?, ?, ?)`,
      args: ['AMGProdZ', hash, 'dev@aethoria.gg', 'developer', 999999, 1],
    });
  ...
  // Seed base cosmetics
  const cosCount = await db.execute({ sql: 'SELECT COUNT(*) as c FROM cosmetics', args: [] });
  if (cosCount.rows[0].c === 0) {
    const cosmetics = [
      ['PYROS','Ember Recolor','skin',300,'common','Warm ember tones','{"primary":"#FF6B35","secondary":"#FFD700"}'],
      ['PYROS','Inferno King','skin',800,'rare','Blazing royalty skin','{"primary":"#FF0000","secondary":"#8B0000"}'],
      ['GLACIS','Blizzard Recolor','skin',300,'common','Icy blue palette','{"primary":"#ADD8E6","secondary":"#FFFFFF"}'],
      ['GLACIS','Frost Queen','skin',800,'rare','Crystalline queen skin','{"primary":"#00FFFF","secondary":"#E0FFFF"}'],
      ['TEMPEST','Storm Recolor','skin',300,'common','Dark stormy palette','{"primary":"#808080","secondary":"#C0C0C0"}'],
      ['TERRA','Stone Recolor','skin',300,'common','Ancient stone tones','{"primary":"#8B7355","secondary":"#A0522D"}'],
      ['LUMIS','Radiant Recolor','skin',300,'common','Golden glow palette','{"primary":"#FFD700","secondary":"#FFFACD"}'],
      ['UMBRA','Void Recolor','skin',300,'common','Deep void tones','{"primary":"#2F0A4B","secondary":"#6A0DAD"}'],
      ['VOLTIS','Static Recolor','skin',300,'common','Electric yellow','{"primary":"#FFFF00","secondary":"#FFD700"}'],
      ['AQUA','Ocean Recolor','skin',300,'common','Deep ocean palette','{"primary":"#000080","secondary":"#4169E1"}'],
      ['SYLVAN','Decay Recolor','skin',300,'common','Corrupted nature tones','{"primary":"#556B2F","secondary":"#8B4513"}'],
      ['DUSK','Ancient Recolor','skin',300,'common','Aged scholar tones','{"primary":"#4B0082","secondary":"#8B008B"}'],
      ['CINDER','Magma Recolor','skin',300,'common','Volcanic red','{"primary":"#8B0000","secondary":"#FF4500"}'],
      ['ALL','Rift Shard Trail','trail',500,'uncommon','Leave rift shards as you move','{}'],
      ['ALL','Null Explosion','hiteffect',600,'uncommon','Void explosion on hits','{}'],
      ['ALL','Developer Aura','aura',0,'special','AMGProdZ exclusive aura','{}'],
    ];
    ...
    console.log('[DB] Cosmetics seeded');
  }
...
export async function createUser(username, passwordHash, email) {
  const r = await db.execute({
    ...
    args: [username, passwordHash, email || null],
  });
